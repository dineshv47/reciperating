...
if ($_POST && $_POST['register-form-submit'] != '') {

	$message = "";
	$message_type = '';

	if ($_POST['fname'] == '') {
		$errors['fname'] = 'Please enter fullname';
	}
	if ($_POST['email'] == '') {
	...
	if ($_POST['repassword'] == '') {
		$errors['repassword'] = 'Please enter repassword';
	}
	if ($_POST['password'] && $_POST['repassword'] && $_POST['password'] !== $_POST['repassword']) {
		$errors['matchPassword'] = 'Password and repassword should match';
	}

	$sqlEmail = "SELECT * FROM users WHERE email = '" . $_POST['email'] . "'";
	$result = $conn->query($sqlEmail);

	if ($result->num_rows > 0) {
		// output data of each row
		$errors['duplicationEmail'] = 'Duplicate Email ID Choose another one!';
	}

	//print_r($errors);
	if ($errors['fname'] == '' && $errors['email'] == '' && $errors['phone'] == '' && $errors['password'] == '' && $errors['repassword'] == ''  && $errors['matchPassword'] == '' && $errors['duplicationEmail'] == '') {

		$sql = "INSERT INTO users (fname, email, phone, password) VALUES ('" . $_POST['fname'] . "','" . $_POST['email'] . "'," . $_POST['phone'] . ",'" . md5($_POST['password']) . "')";
		if ($conn->query($sql) === TRUE) {
			$message = "Registered successfully <a href=" . BASE_URL . "login.php>Login</a>";
...
<body class="stretched">
	...
	<div id="wrapper" class="clearfix">
		...
		<section id="content">
			<div class="content-wrap" style="background: url('images/bg.jpg') center center no-repeat; background-size: cover;">
							...
							<div class="tab-content" id="tab-register">
								<div class="card mb-0">
									<div class="card-body" style="padding: 40px;">
										...
										?>
										<form id="register-form" name="register-form" class="row mb-0" action="" method="post" autocomplete="off">

											<div class="col-12 form-group">
												<label for="register-form-name">Name:</label>
												<input type="text" id="register-form-name" name="fname" value="<?php echo (!empty($_POST) && $_POST['fname'] != '') ? $_POST['fname'] : '' ?>" class="form-control" autocomplete="fdsf" />
												<?php if ($errors['fname'] != '') {
												?>
											...
											<div class="col-12 form-group">
												...
												<input type="text" id="register-form-email" name="email" value="<?php echo (!empty($_POST) && $_POST['email'] != '') ? $_POST['email'] : '' ?>" class="form-control" autocomplete="offdff" />
												<?php if ($errors['email'] != '') {
												?>
													<span class="error"><?php echo $errors['email'] ?></span>
												<?php
												} ?>
												<?php if ($errors['duplicationEmail'] != '') {
												?>
													<span class="error"><?php echo $errors['duplicationEmail'] ?></span>
												<?php
												} ?>
											</div>
											...
											<div class="col-12 form-group">
												...
												<input type="text" id="register-form-phone" name="phone" value="<?php echo (!empty($_POST) && $_POST['phone'] != '') ? $_POST['phone'] : '' ?>" class="form-control" autocomplete="offdfsf" />
												<?php if ($errors['phone'] != '') {
												?>
											...
											<div class="col-12 form-group">
												...
												<input type="password" id="register-form-password" name="password" value="" class="form-control" autocomplete="ofdsfdff" />
											...
											<div class="col-12 form-group">
												...
												<input type="password" id="register-form-repassword" name="repassword" value="" class="form-control" autocomplete="offdfdsff" />
												<?php if ($errors['repassword'] != '') {
												?>
													<span class="error"><?php echo $errors['repassword'] ?></span>
												<?php
												} ?>
												<?php if ($errors['matchPassword'] != '') {
												?>
													<span class="error"><?php echo $errors['matchPassword'] ?></span>
												<?php
												} ?>
											</div>
											...
											<div class="col-12 form-group">
												...
												<a href="<?php echo BASE_URL; ?>login.php" class="float-end">Already user? Login</a>
											</div>
